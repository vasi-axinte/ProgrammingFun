...
namespace Theory.Stack
{
    public class PillarOfBooks    
    {
        ...
        {     //Stack accepts null as a valid value and allows duplicate elements
            Stack<string> books = new Stack<string>(); //Creating a stack
            books.Push("book1");  //push adds elements to our stack
            books.Push("book2");
            books.Push("book3");
            books.Push("book4");
            books.Push("book5");

            foreach(var book in books) //We are accesing the elements of Recipe(our queue) using foreach loop
            {
            ...
            Console.WriteLine("The book from the top of the pillar is {0} ", books.Peek()); // Peek returns the element that is at the top of the stack withour removing it
            Console.WriteLine();
            ...
            Console.WriteLine("Now we are at the book that we wanted: {0} ",books.Peek()); // Peek returns the element that is at the top of the stack withour removing it
            Console.ReadLine();
        }
    }
}